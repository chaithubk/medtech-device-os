...
while (( RETRY < MAX_RETRY )); do
    if ssh -o ConnectTimeout=2 -o BatchMode=yes -o StrictHostKeyChecking=no \
           -p 2222 medadmin@localhost "echo 'SSH ready'" >/dev/null 2>&1; then
...
if (( RETRY >= MAX_RETRY )); then
    ...
    echo "  ssh -p 2222 medadmin@localhost"
    exit 1
fi

echo ""
echo "=== System Information ==="
ssh -o StrictHostKeyChecking=no -p 2222 medadmin@localhost "uname -a"

echo ""
echo "=== Service Status ==="
ssh -o StrictHostKeyChecking=no -p 2222 medadmin@localhost "systemctl list-units --type=service --state=running | grep medtech"

echo ""
echo "=== MQTT Broker Check ==="
ssh -o StrictHostKeyChecking=no -p 2222 medadmin@localhost "systemctl status mosquitto | head -5"

echo ""
echo "=== Available MQTT Topics (subscribe for 5 seconds) ==="
timeout 5 ssh -o StrictHostKeyChecking=no -p 2222 medadmin@localhost \
